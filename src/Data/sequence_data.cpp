...
SequenceData::SequenceData(ConfReader *confReader) {
	string inputFile = confReader->getString("seqdata_input_file");
	string outputFile = confReader->getString("seqdata_output_file");

	numData = confReader->getInt("seqdata_sample_num");	

	m_inputSeqLen = confReader->getInt("seqdata_input_len");
	m_outputSeqLen = confReader->getInt("seqdata_output_len");

	m_inputDim = confReader->getInt("seqdata_input_dim");
	m_outputDim = confReader->getInt("seqdata_output_dim");

	m_input = new float[numData * m_inputSeqLen * m_inputDim];
	m_output = new float[numData * m_outputSeqLen * m_outputDim];

    ifstream inputfile (inputFile.c_str(), ios::in|ios::binary);
    if (inputfile.is_open()) {
        inputfile.seekg (0, ios::end);
        int size = inputfile.tellg();
        if (size != sizeof(float) * numData * m_inputSeqLen * m_inputDim) {
    ...
    if (outputfile.is_open()) {
        ...
        if (size != sizeof(float) * numData * m_outputSeqLen * m_outputDim) {
